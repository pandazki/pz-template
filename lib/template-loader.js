...
class TemplateLoader {
  ...
  async prepare () {
    ...
    if (isGitUrl(this.location)) {
      ...
      this.hash = (await simpleGit(tmp).raw('rev-parse', 'HEAD')).trim()
  ...
  async copyTo (target, meta) {
    await fs.copy(this.selectedConfig.tplDir, target);
    if (this.hash) {
      const data = {
        meta,
        location: `${this.repo}|${this.hash}`
      }
      await fs.writeFile(path.resolve(target, ".tplver_" + this.selectedConfig.name), JSON.stringify(data, undefined, 2))
    }
  }
}
